...
class MenTestRankingFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewLifecycleOwner.lifecycleScope.launch(Dispatchers.Default) {
            val apiResponseResult: Result<List<RankingTeam>?> = viewModel.getTeamRanking("TEST","men")
       ...
       /* viewModel.readAllRanking.observe(viewLifecycleOwner) {
            Log.d("Rank", "${it.size} ")
            recyclerView.layoutManager=LinearLayoutManager(requireContext())
            val adapterViewState = recyclerView.layoutManager?.onSaveInstanceState()
            recyclerView.layoutManager?.onRestoreInstanceState(adapterViewState)
            recyclerView.adapter = MenRankingAdapter(
